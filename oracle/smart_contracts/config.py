            ...
            if deploy_response.action_taken in [
                ...
                if not suggested_params.flat_fee:
                    if transaction.fee > params.max_fee_micro_algos:
                        raise Exception(
                            f"Cancelled transaction due to high network congestion fees. "  # noqa: E501
                            f"Algorand suggested fees would cause this transaction to cost {transaction.fee} µALGOs. "  # noqa: E501
                            f"Cap for this transaction is {params.max_fee_micro_algos} µALGOs."  # noqa: E501
                        )
                    if transaction.fee > MIN_TXN_FEE:
                        logger.warning(
                            f"Algorand network congestion fees are in effect. "
                            f"This transaction will incur a fee of {transaction.fee} µALGOs."  # noqa: E501
                        )
